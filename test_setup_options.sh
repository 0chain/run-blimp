...
if grep -qE '^_gwcpu=\$\(nproc' "$RC" && grep -qF 'EC_CONC=$_gwcpu' "$RC" && ! grep -qF '_warpconc' "$RC"; then
  ok "warp concurrency = gateway vCPUs (1x), identical to the panel"
else
  bad "warp concurrency is not 1x-vCPU — CLI and panel numbers diverge"
fi
