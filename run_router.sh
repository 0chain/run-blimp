...
echo "[cache-bench] origin=s3://$BKT/$TABLE/  region=$REGION  router=$ROUTER  par=$PAR"

# --- enumerate the object set (key<TAB>size), optionally capped to MAX_BYTES -------
MAP=$(aws s3 ls "s3://$BKT/$TABLE/" --recursive --region "$REGION" 2>/dev/null | awk '$3+0>0{print $4"\t"$3}')
[ -n "$MAP" ] || { echo "no objects under s3://$BKT/$TABLE/ (creds/endpoint?)"; exit 1; }
...
if command -v mount-s3 >/dev/null 2>&1 && [ -n "${GW_AK:-}" ]; then
  ...
  if AWS_ACCESS_KEY_ID="$GW_AK" AWS_SECRET_ACCESS_KEY="$GW_SK" mount-s3 --endpoint-url "http://$GW:9000" --region us-east-1 "$BKT" "$MPS3" 2>/dev/null; then
    t0=$(now); echo "$KEYS" | xargs -P"$PAR" -I{} dd if="$MPS3/{}" of=/dev/null bs=1M 2>/dev/null
    t1=$(now); WARM_MPS3=$(mbps "$BYTES" "$(el "$t0" "$t1")")
    fusermount -u "$MPS3" 2>/dev/null || true
    echo "  cache-mp-s3: $WARM_MPS3 MB/s (mountpoint-s3 over cache bucket)"
  fi
fi

# ============================ 4) CACHE-NFS (warm) ================================
# The same cached objects live at <MNT>/<BKT>/<key> (the cache bucket is a gateway S3
# bucket == NFS namespace). Read them via Ganesha to /dev/null.
mount_nfs
MISS=$(echo "$KEYS" | awk -v m="$MNT/$BKT" '{ "test -f \""m"/"$0"\" && echo hit || echo miss" | getline r; if(r=="miss")c++ } END{print c+0}')
[ "${MISS:-0}" -gt 0 ] && echo "  (note: $MISS/$NOBJ objects not present on NFS — router may cache lazily; reading the hits)"
t0=$(now)
echo "$KEYS" | xargs -P"$PAR" -I{} sh -c 'dd if="$1/$2" of=/dev/null bs=1M 2>/dev/null' _ "$MNT/$BKT" "{}"
t1=$(now); WARM_NFS=$(mbps "$BYTES" "$(el "$t0" "$t1")"); WARM_NFS_S=$(el "$t0" "$t1")
echo "  cache-NFS : $WARM_NFS MB/s (${WARM_NFS_S}s, eblobber cache via Ganesha)"

# ================================ summary ========================================
echo ""
echo "==================== read-through cache: $TABLE ($(awk -v b="$BYTES" 'BEGIN{printf "%.1f",b/1073741824}') GiB) ===================="
...
echo "  NOTE: if the set (< gateway RAM) fits in page cache the warm numbers include"
echo "        gateway-side caching; for a cold-from-eblobber number use a set > gateway RAM."
